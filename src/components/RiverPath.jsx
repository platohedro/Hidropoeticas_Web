        ...
        {existingSections.map((sectionId) => {
          ...
          return (
            ...
            >
              <a 
                href={`#${sectionId}`}
                className={`w-6 h-6 rounded-full border-2 transition-all duration-300 pointer-events-auto relative
                  ${isActive 
                    ? "bg-white border-white scale-125" 
                    : "hover:bg-white hover:border-white hover:scale-110"}`}
                style={{
                  backgroundColor: isActive ? "white" : "#0ea5e9",
                  borderColor: isActive ? "white" : "#0ea5e9",
                  boxShadow: isActive ? "0 0 15px rgba(6, 182, 212, 0.5)" : "none"
                }}
                onMouseEnter={() => setIsHovered(sectionId)}
              ...
              >
                <span 
                  className={`absolute left-1/2 -bottom-8 transform -translate-x-1/2 text-cyan-100 text-sm font-medium tracking-wider whitespace-nowrap transition-all duration-300 px-3 py-1 rounded-full backdrop-blur-sm
                    ${isActive || isHovered === sectionId ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
                  style={{
